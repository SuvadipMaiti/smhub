...
import Wallet from '../../models/mongodb/Wallet.js';
...
export const register = asyncHandler(async (req, res) => {
  ...
  if (registerUser) {
    var id = new mongoose.Types.ObjectId(registerUser._id);
...
export const login = asyncHandler(async (req, res) => {
  ...
  if (loginUser) {
    if (await bcrypt.compare(req.body.password, loginUser.password)) {
      var id = new mongoose.Types.ObjectId(loginUser._id);
      const checkWallet = await Wallet.findOne({ userId: id }); 
...
export const logingoogle = asyncHandler(async (req, res) => {
  ...
  if (loginUser) {
    var id = new mongoose.Types.ObjectId(loginUser._id);
    var checkWallet = await Wallet.findOne({  userId: id });
    if (!checkWallet?.userId) {
      const newWallet = await Wallet.create({
        userId: id,
      });
...
export const loginfacebook = asyncHandler(async (req, res) => {
  ...
  if (loginUser) {
    var id = new mongoose.Types.ObjectId(loginUser._id);
    checkWallet = await Wallet.findOne({ userId: id });
    if (!checkWallet?.userId) {
      const newWallet = await Wallet.create({
        userId: id,
      });
    ...
    return res.status(200).json({
      status: false,
      message: 'Registration through facebook complete.',
    });
  }
});
...
export const auth = asyncHandler(async (req, res) => {
  if (req.user && req.user._id && req.token) {
